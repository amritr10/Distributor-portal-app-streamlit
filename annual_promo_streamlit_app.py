...
def reset_all_filters():
    # only clear the filters you genuinely have widgets for now
    reset_keys = [
        "search_query", "search_bar_input",
        "selected_category", "selected_series",
        # note: we no longer manage selected_lifecycle in state
    ]
    ...
    # reset product‐group back to the one guaranteed to exist
...
if not st.session_state.logged_in:
    ...
    with st.form("login_form"):
        ...
        if submitted:
            ...
            # 2) domain‐check
            elif not any(email.strip().lower().endswith(d) for d in [
                         "@cuttedge.com.au",
                ...
                st.error(
                  "Your're not allowed to access this promotion. Please contact Amrit to make sure you are on the list."
                )
            # 3) password‐check
            elif password.strip() != "dist2025":
                st.error("Incorrect password. Please try again.")
            # 4) success → record login
            else:
                ...
                try:
                    ...
                    # ensure “Last login” column exists
                    if "Last login" not in users_df.columns:
                        users_df["Last login"] = ""

                    # update timestamp if email exists, else append new row
                ...
                # finally mark logged in and rerun
...
# ------------------------- Promotion Header -------------------------
st.markdown('<div class="promo-header"><h1>Distributor kick off New Products 2025</h1></div>', unsafe_allow_html=True)
...
# ------------------------- Load Product Data -------------------------
csv_file = "key_products.csv"
df = pd.read_csv(csv_file)
df = df.dropna(subset=["Category", "Series"])

# ──────────────────────────────────────────────────────────
# STEP 1: only keep New Product life-cycle items  
if "Product life cycle" in df.columns:
    df = df[df["Product life cycle"] == "New Product"].copy()
# ──────────────────────────────────────────────────────────
...
#     )
# STEP 2: no lifecycle control any more, we know it's always New Product
selected_lifecycle = "New Product"

# shift your layout to just 2 cols now
...
if "Product Group" in df.columns:
    ...
    product_group_options = ["All Product Groups"] + product_groups

    # sanity-check the session state before we render the widget
    if st.session_state.get("selected_product_group") not in product_group_options:
        st.session_state["selected_product_group"] = product_group_options[0]
...
# auto-sync Category whenever the user has picked one or more Series
sel_series = st.session_state.get("selected_series", [])
if sel_series:
    # find all categories for the picked series
    cats = (
        df[df["Series"].isin(sel_series)]
        ["Category"]
        .dropna()
        .unique()
        .tolist()
    ...
    if len(cats) == 1:
        # exactly one category ⇒ force that
        st.session_state["selected_category"] = cats[0]
    else:
        # mixed or no match ⇒ reset to “All Categories”
        st.session_state["selected_category"] = "All Categories"
        ...
        if new_series_list:
            ...
            if series_info_df.empty:
                ...
                if not exp_products_df.empty:
                    ...
                    cols_to_show = ["Name", "Description", "List Price", "Distributor Price", "Buy URL"]
